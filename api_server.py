...
# ── 文档解析 Prompts ──────────────────────────────────────────────
PARSE_PROMPTS = {
    "credit_report": """你是中国银行贷款审批专家，请从这份人行征信报告中提取信息。

【第一步：确定报告基准日期】
找到报告时间（如"2026-04-07"），后续所有日期计算以此为基准。

【年龄计算规则（非常重要）】
身份证号第7-14位是完整出生日期，格式 YYYYMMDD，共8位数字。
- 第7-10位 = 出生年份（4位完整年份，如1973、1985、1996）
- 第11-12位 = 出生月份
- 第13-14位 = 出生日
计算方法：年龄 = 报告年份 - 出生年份，若报告日期的月日 < 出生月日则再减1。
例：身份证 362301197305120034，出生年份=1973，报告2026年=53岁（若生日已过）或52岁。
严禁把出生年份理解成2位数（如把"73"算成2073年）。

【第二步：统计查询次数（最重要，必须逐条核对）】
在"机构查询记录明细"表格中，逐行扫描每一条记录：
- 只有查询原因 = "贷款审批" 或 "信用卡审批" 的才计入
- 以下原因绝对不计入：贷后管理、担保资格审查、法人代表/负责人/高管等资信审查、本人查询
- 近6个月范围：报告日期往前推6个整月（如报告2026-04-07，则统计2025-10-07及之后）
- 近2年范围：报告日期往前推24个整月

请将所有符合"贷款审批"或"信用卡审批"的记录逐一列出，然后按日期范围分别统计：
  inquiries_6m = 其中日期在近6个月内的条数
  inquiries_2y = 其中日期在近2年内的条数

【第三步：基本信息】
- name: 报告顶部姓名
- age: 身份证第7-14位=出生日期YYYYMMDD，精确计算年龄（注意生日是否已过）
- job_type判断（优先级从高到低）：
  1. 若"相关还款责任信息"中此人为某私营/贸易/科技公司的"保证人" → "self"（企业主/自雇）
  2. 若有明确政府/机关/事业单位信息 → "gov"
  3. 若有国有企业信息 → "state"
  4. 无法判断 → "private"

【第四步：信用卡（仅人民币账户）】
逐张统计：账户描述含"美元"或"外币"的跳过；"人民币账户"的计入。
- 已使用额度：优先用"已使用额度"字段；若无此字段只有"余额"则用余额
- 分别加总人民币卡的授信额度和已使用额度

【第五步：房产贷款】
贷款中有"个人住房贷款"或"个人商用房（含商住两用）贷款" → has_property=true
- 计算贷款总期数：发放日到到期日之间月数
- 估算月供：原始贷款金额 ÷ 总期数（等额本金粗估）
- property_loan = 当前余额

【第六步：其他未结清贷款】
统计非房产类未结清贷款，计算每笔的剩余月数（到期日-报告日）和估算月供（余额÷剩余月数）

【第七步：existing_monthly_payment】
= 房贷估算月供 + 所有其他未结清消费贷估算月供之和

输出格式（只输出JSON，不含任何其他文字）：
{
  "name": 姓名,
  "age": 年龄（整数）,
  "job_type": "gov"/"state"/"private"/"self"/null,
  "company_name": 担保公司名称（无则null）,
  "inquiries_6m": 近6个月贷款审批+信用卡审批次数（整数）,
  "inquiries_2y": 近2年贷款审批+信用卡审批次数（整数）,
  "overdue_count": 逾期次数（"发生过逾期的账户数"为"--"则为0，整数）,
  "overdue_90d": 90天以上逾期（"发生过90天以上逾期的账户数"为"--"则false，布尔）,
  "credit_card_used": 人民币信用卡已使用额度合计（元，数字）,
  "credit_card_limit": 人民币信用卡授信额度合计（元，数字）,
  "has_property": 有住房或商用房贷款（布尔）,
  "property_loan_total": 房贷原始金额（元，无则0）,
  "property_loan": 房贷当前余额（元，无则0）,
  "property_loan_months": 房贷总期数（月数，整数，无则null）,
